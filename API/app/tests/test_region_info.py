...
class TestSuccessfulCases:
    ...
    def test_get_region_info(self, client):
        ...
        response = client.get(f"/region-info/?id={ID}&year={YEAR}")
...
class TestFailureCases:
    ...
    def test_data_lack(self, client):
        ...
        no_data_response = client.get("/region-info/")
        assert no_data_response.status_code == StatusCode.ValidationError

        no_id_response = client.get(f"/region-info/?year={YEAR}")
        assert no_id_response.status_code == StatusCode.ValidationError

        no_year_response = client.get(f"/region-info/?id={ID}")
    ...
    def test_wrong_year_value(self, client):
        ...
        too_little_year_response = client.get(f"/region-info/?id={ID}&year={too_little_year}")
        assert too_little_year_response.status_code == StatusCode.ValidationError

        too_big_year_response = client.get(f"/region-info/?id={ID}&year={too_big_year}")
    ...
    def test_wrong_id_value(self, client):
        ...
        too_little_id_response = client.get(f"/region-info/?id={too_little_id}&year={YEAR}")
        assert too_little_id_response.status_code == StatusCode.ValidationError

        too_big_id_response = client.get(f"/region-info/?id={too_big_id}&year={YEAR}")
