...
class TestSuccessCases:
    ...
    def test_get_all_data(self, client):
        ...
        all_data_response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}&is_by_district={IS_BY_DISTRICT}&aggregation_type={AGGREGATION_TYPE}&use_filter={USE_FILTER}&min_filter_value={MIN_FILTER_VALUE}&max_filter_value={MAX_FILTER_VALUE}")
    ...
    def test_get_data_without_filter(self, client):
        ...
        no_filter_response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}&is_by_district={IS_BY_DISTRICT}&aggregation_type={AGGREGATION_TYPE}")
    ...
    def test_get_data_by_region(self, client):
        ...
        by_region_response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}")
...
class TestFailureCases:
    ...
    def test_data_lack(self, client):
        ...
        no_data_response = client.get("/feature-info/")
        assert no_data_response.status_code == StatusCode.ValidationError

        no_feature_response = client.get(f"/feature-info/?year={YEAR}")
        assert no_feature_response.status_code == StatusCode.ValidationError

        no_year_response = client.get(f"/feature-info/?feature={FEATURE}")
        assert no_year_response.status_code == StatusCode.ValidationError

        no_aggregation_type_response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}&is_by_district={IS_BY_DISTRICT}")
        assert no_aggregation_type_response.status_code == StatusCode.ValidationError

        no_filter_value_response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}&use_filter={USE_FILTER}")
    ...
    def test_wrong_aggregaion_type(self, client):
        ...
        response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}&is_by_district={IS_BY_DISTRICT}&aggregation_type={wrong_aggregation_type}")
    ...
    def test_wrong_filter_value(self, client):
        ...
        wrong_max_value_response = client.get(f"/feature-info/?feature={FEATURE}&year={YEAR}&use_filter={USE_FILTER}&max_filter_value={wrong_max_value}")
